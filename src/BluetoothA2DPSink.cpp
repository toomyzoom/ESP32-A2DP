...
void BluetoothA2DPSink::app_task_start_up(void)
{
    ...
    if (app_task_handle==NULL) {
        if (xTaskCreatePinnedToCore(ccall_app_task_handler, "BtAppT", 4096, NULL, task_priority, &app_task_handle, 0) != pdPASS){
...
void BluetoothA2DPSink::audio_data_callback(const uint8_t *data, uint32_t len) {
    ...
    if (is_i2s_output) {
        ...
        if (this->i2s_config.mode & I2S_MODE_DAC_BUILT_IN) {
            sToU.update_audio_data((uint8_t*)data, len, codec_bps);
        }    
...
void BluetoothA2DPSink::av_hdl_avrc_tg_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_TG_SET_ABSOLUTE_VOLUME_CMD_EVT: {
        ESP_LOGD(BT_AV_TAG, "AVRC set absolute volume: %d%%", (int)rc->set_abs_vol.volume * 100/ 0x7f);
